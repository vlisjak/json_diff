...
def using_deepdiff(left, right):
  ...
  dd_tree = DeepDiff(left, right, ignore_order=True, report_repetition=True, view="tree", threshold_to_diff_deeper=0)
...
def using_difflib(left, right):
  ...
  def get_edits_string(old: str, new: str) -> str:
      ...
      lines = difflib.unified_diff(old.splitlines(keepends=True), new.splitlines(keepends=True), fromfile="before", tofile="after")
